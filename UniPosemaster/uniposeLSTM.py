...
class Trainer(object):
    ...
    def training(self, epoch):
        ...
        for i, (input, heatmap, centermap, img_path) in enumerate(tbar):
            ...
            print("c:",centermap_var.shape)
    

            self.optimizer.zero_grad()
         
            heat = torch.zeros(1,self.numClasses+1, 46, 46).cuda()
            cell = torch.zeros(1,15, 46 , 46).cuda()
            hide = torch.zeros(1,15, 46, 46).cuda()
...
if args.dataset == 'Penn_Action':
    args.train_dir  = r'D:\CEG\FYP\UniPose-master\Penn_Action\Penn_Action\\'
    args.val_dir    = r'D:\CEG\FYP\UniPose-master\Penn_Action\Penn_Action\\'
