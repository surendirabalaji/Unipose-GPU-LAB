...
class Penn_Action(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        data   = loadmat(os.path.join(self.data_dir, frames))
...
#         boxes      = torch.zeros(self.seqTrain, 5, self.height, self.width)  # [3,368,368]
        ...
        for i in range(self.seqTrain):
            # read image
            img_path = os.path.join('Penn_Action\Penn_Action\\frames\\'+framespath[:-4],\
            ...
            #img_path  = self.frame_dir + variable['img_paths']
...
#             bbox      = np.load(self.labels_dir + "BBOX/" + variable['img_paths'][:-4] + '.npy')
            ...
            for part in range(0, 13):  # for each part
                if self.isNotOnPlane(label[0, part, i], label[1, part, i],dim[0][1],dim[0][0]):
                    label[2, part, i] = -1
